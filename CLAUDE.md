...
An AI calendar assistant: chat in natural language to create/move/cancel events, and ask for **live stock & market data**. Two Railway services + Postgres.
...
- **Frontend** — React 18 + Vite + Tailwind SPA (`frontend/`). Chat panel (STOMP/SockJS over `/ws`), a calendar with client-side recurrence expansion, a live `MarketTicker`, and `LoginScreen` (Google + email/password).
...
- `/api/**` requires authentication **except**: `/api/auth/**`, `/api/market/**`, `/actuator/health`, `/ws/**`, `/oauth2/**`, `/login/**`. Unauthenticated API calls get **401** (not an OAuth redirect).

## Market / stock data

- `MarketDataService` fetches quotes from **Yahoo Finance v8 chart** (keyless, `query1`→`query2` fallback), with an optional `MARKET_FINNHUB_API_KEY` fallback for single stocks. 60-second in-memory cache.
- Surfaces: agent tools `getStockQuote(symbol)` + `getMarketSummary()`; REST `GET /api/market/summary` and `GET /api/market/quote?symbol=AAPL`; the UI `MarketTicker`.
- The agent's scope is **calendar + markets only** — it declines unrelated requests and does **not** give financial advice (see the system prompt in `AgentService.buildSystemPrompt`).
- Keep market data live: the default source is keyless and needs no config. If Yahoo egress is ever blocked from Railway, set `MARKET_FINNHUB_API_KEY` on the backend service.
...
2. Verify: the market ticker shows live indices; chat creates/lists events; ask "how is AAPL doing?" and "how's the market today?" and confirm the agent returns live numbers; the calendar renders created events.
3. Confirm isolation: a second fresh account sees an empty calendar (data is per-user).

## Standing instructions from the owner

- Keep the app able to retrieve the latest market/stock info.
