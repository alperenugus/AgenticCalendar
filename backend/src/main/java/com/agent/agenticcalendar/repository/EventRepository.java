...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    // Get events that start before a given time (for recurring event expansion)
    // This includes both non-recurring events and recurring events that might have occurrences
    List<Event> findBySessionIdAndStartTimeBefore(String sessionId, LocalDateTime endTime);
    
    List<Event> findByGoogleUserIdAndStartTimeBefore(String googleUserId, LocalDateTime endTime);
}
