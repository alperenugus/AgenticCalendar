...
**This pass fixed that** (see §4) alongside delivering the two requested features (market/stock data + username/password login). The remaining recommendations in §5 are real but were deliberately deferred to avoid a large, blind, autonomous rewrite of working code — they're written up so they can be done deliberately.
...
React SPA (Vite, Tailwind)                 Spring Boot 3.4 (Java 21)
 ...
 └─ MarketTicker ─ GET /api/market/summary ─└─ MarketController ─ MarketDataService → Yahoo Finance
```
...
| Area | Change |
|---|---|
| **Feature: market/stock data** | `MarketDataService` (keyless Yahoo Finance v8 chart; `query1`→`query2` fallback; optional `MARKET_FINNHUB_API_KEY`; 60 s cache). `getStockQuote`/`getMarketSummary` agent tools wired into the ReAct `switch` + system prompt (scope relaxed from calendar-only to **calendar + markets**, with an explicit "no financial advice" guard). `/api/market/{summary,quote}` REST endpoints. A live `MarketTicker` bar (S&P 500 / Dow / Nasdaq) in the UI. |
...
| **P0: authorization** | `CurrentUser` derives the owner key/email **server-side from the authenticated principal**; the `X-Google-User-Id`/`X-Google-User-Email` headers and `?googleUserId` param are no longer trusted. `/api/**` now requires authentication except `/api/auth/**`, `/api/market/**`, `/actuator/health`, `/ws/**`, `/oauth2/**`, `/login/**`. API calls return **401** (not an OAuth redirect). `EventController` scopes every query to the authenticated owner and checks ownership on by-id reads; the `findAll()` fallback that leaked all users' events was removed. |
